...
~~~ sh
sudo apt-get install htop ranger neofetch cmatrix nano vim hollywood sl figlet locate;
...
- If you don't have a `picom.conf` file, you can copy one form the repository in `I3wm/picom/picom.conf`.
- And finally do this command so that picom always runs.

~~~sh
echo exec_always --no-startup-id picom --experimental-backends >> ~/.config/i3/config
~~~
